...
> 一份从问题出发的多智能体教学讲义（完善版）
> 适用对象：已有单 Agent 使用经验、准备设计或评审多智能体结构的工程师与讲师
> 建议时长：50 分钟授课 + 20 分钟演示讨论
...
只有左边三项中的至少一项足够明显，而且大于协调成本时，多智能体才值得使用。这个公式不是精确度量，而是一张"决策前的体检表"：每次想拆 Agent 时，先问自己收益来自哪一项、协调成本又是什么。
...
> **课堂提问（开场热身）**：请大家回忆最近一次想让“多个 AI 分工”的场景——当时想解决的到底是能力问题、上下文问题，还是只是“感觉人多力量大”？

---

## 1. 为什么需要多智能体

多智能体只有三类正当动机：**能力互补、上下文（含权限）隔离、独立任务并行**。本节逐一展开，并给出反面清单。

### 1.1 原因一：单个 Agent 存在能力边界

一个 Agent 能做什么，不只取决于它的提示词，还取决于四类资源：

- **模型能力**：使用的模型支持什么模态、多长上下文、什么推理水平；
- **工具**：能调用哪些外部能力（搜索、执行、OCR、裁剪等）；
- **上下文**：能读取哪些信息；
- **权限**：被允许做哪些操作。
...
这样，Primary 消费的是一份带有来源边界的“观察报告”，而不是把视觉模型的一切判断都当成事实。注意 `uncertainties` 和 `limitations` 两个字段：它们让 Primary 知道哪些结论不能从这张图片得出，避免把“看不见”误当成“不存在”。
...
| 特征 | 模型调用 | Agent 调用 |
| --- | --- | --- |
| 任务说明 | 一次性 prompt | 稳定的系统指令与职责 |
| 输入输出 | 自由文本 | 结构化契约 |
| 上下文 | 与调用方混杂 | 独立、受控 |
| 工具使用 | 通常无 | 可按需使用 |
| 终止判断 | 返回即结束 | 能判断完成 / 失败 / 受阻 |
...
- **锚定**：生成阶段形成的假设会继续影响验收阶段；
- **辩护倾向**：Agent 更容易解释自己的方案，而不是主动寻找反例；
- **盲区延续**：生成时忽略的边界条件，验收时也可能继续忽略；
- **标准稀释**：长上下文中混杂了设计讨论、失败尝试和临时判断，验收标准反而不突出；
- **假阳性**：验收看起来完成了，但真实路径可能仍是 fallback、mock、placeholder 或 hard-code。
...
这里要注意：**上下文隔离不等于让 Reviewer 什么都不知道**。Reviewer 可以不知道 Builder 的思维过程，但必须知道任务目标、约束和验收标准。否则它不是独立验收，而是盲审——盲审出的 FAIL 没有修复价值，盲审出的 PASS 没有可信度。

> **课堂提问**：如果把 Builder 的完整对话记录都交给 Reviewer，会发生什么？（引导学员说出“锚定”与“叙事污染”——Reviewer 会不自觉沿着实现者的解释路径走。）
...
并行的主要收益有两个：一是**缩短墙上时钟时间**；二是在上下文隔离的前提下，获得**多个真正独立的证据源**——这比“让一个 Agent 多想几遍”更接近独立判断。

但“可以拆成多个描述”不等于“可以并行”。如果 B 的输入依赖 A 的结果，强行并行只会产生猜测、返工和冲突。判断能否并行的试金石是：**每个子任务在启动那一刻，输入是否已经完整？**
...
尤其需要警惕最后一种情况：单 Agent 解决不了的无边界、不可验证问题，拆给多个 Agent 后并不会自然变得可验证，反而可能得到多份各自自洽但互相冲突的答案。**不可验证性不会随着 Agent 数量增加而消失，只会被稀释得更难发现。**
...
多个 Skill 可以组合，使一个 Agent 覆盖多个场景。它们通常**共享**这个 Agent 的当前上下文、模型和权限——这是 Skill 与 Subagent 最本质的区别：Skill 改变的是“做法”，不是“执行边界”。
...
反过来说，如果两个“Agent”在模型、工具、权限、上下文上完全一致，那它们大概率是同一个 Agent 的两个名字。
...
- **控制上下文**：子 Agent 只看到它需要的材料；
- **限定行为边界**：明确可以做和不可以做的事；
- **保证可消费性**：返回结果能被程序或 Primary 稳定解析。

一个实用的心法：**把任务包当作给一位聪明但完全不了解前情的同事写的交接单**——它需要的一切都在单子里，单子之外的它一概不知道。
...
`BLOCKED` 很重要。没有它，模型容易在证据不足时被迫猜测一个 PASS 或 FAIL——而猜测出来的结论比没有结论更危险，因为它看起来像一个结论。
...
如果视觉任务只是一次固定转换，这种轻量实现通常已经足够，不必先引入复杂的多智能体框架。**先用最薄的实现验证拆分本身有收益，再决定是否需要框架。**

### 4.4 实际样例（课堂展示位）

> 此处展示一次真实的视觉委托运行截图：左侧为输入图片与任务指令，右侧为返回的结构化 JSON。
> 建议截图包含：① 输入图片与 objective；② 返回的 observations / visible_text / uncertainties / limitations；③ 一次故意使用模糊截图时 uncertainties 字段生效的对照。
...
这里将 Builder 可调用的 Subagent 限定为 `reviewer`，避免 Primary 无限制地继续派生子 Agent——**委托能力本身也需要权限收敛**。
...
> 此处展示 OpenCode 中一次完整的 Builder → Reviewer 委托运行截图。
> 建议截图包含：① `.opencode/agents/` 目录中两个 Agent 文件；② Primary 会话中发起委托的位置；③ child session 中 Reviewer 的独立上下文；④ 一次真实的 FAIL 结论及其 finding 列表。
...
串行模式必须设置最大循环次数。例如两轮验收仍失败，就停止并报告，而不是在 Agent 之间无限往返。**没有最大轮次的闭环不是闭环，是漩涡。**
...
核心原则一句话：**投票处理的是意见分歧，证据处理的是事实判断；验收属于后者。**
...
如果先从“模拟一个软件公司”开始，往往会产生大量角色，却没有真正的边界差异。**角色名是设计的终点，不是起点。**
...
软约束的价值在于引导行为，硬边界的价值在于**让违规行为在机制上不可能发生**。两者应该互补，而不是互相替代。
...
Artifact 能够被保存、复用、重放和审计，也更容易在不同模型或不同 Agent Runtime 之间传递。当协作出现问题时，Artifact 也是唯一可靠的“现场还原材料”。
...
不要只设计“如何继续”，还要设计“什么时候必须停止”。一个只会前进的系统，遇到问题时只会更快地制造更多问题。
...
Agent 数量增加会同时增加调用成本、延迟、失败节点和汇总难度。多智能体应从最小可用结构开始——一个 Primary 加一个 Subagent，往往已经能覆盖大部分真实收益。

### 误区二：不同角色名称等于不同视角

如果不同 Agent 使用相同上下文、相同模型、相同工具和相同提示结构，它们的输出高度相关，并不构成真正独立的判断。相关性高的“多份意见”在统计上接近一份意见。
...
这会重新混合“生成者”和“裁判”的职责。Reviewer 应输出 finding；修改仍由 Builder 完成，然后再次独立验收。裁判改完再吹哨，比赛就失去了意义。
...
> **课堂练习（5 分钟）**：给出一个你自己的真实任务，按上面五个问题逐项回答。只要有一问答不上来，就先回到单 Agent + Skill，把那个问题补清楚再说。
...
这个拆分解决的是**异构能力**问题。视觉 Agent 不需要接管整项任务，只负责它最擅长的感知环节。同时让学员看到：结构化输出里的“不确定”本身就是一种有价值的产出。

#### 实际样例（演示截图位）

> 此处放置演示一的真实运行截图：纯文本 Primary 面对图片的表现、接入视觉 Subagent 后的对比、模糊截图下的 uncertainties 输出。
...
> 此处放置演示二的真实运行截图：Builder 的自评结论、Review Handoff 内容、Reviewer 在 child session 中的独立验收输出（尤其是一次抓到预埋缺陷的 FAIL）。
...
如果一个 Subagent 长期只是重复 Primary 的结论，就应考虑合并它，或者重新设计它的能力、上下文和权限边界。**没有观测就没有优化——多智能体系统的每个边界都应当接受“增量收益”的审计。**
...
1. **边界判别**：从你身边找一个正在使用（或计划使用）多 Agent 的场景，写出每个 Agent 在模型、工具、权限、上下文四个维度上的差异。写不出差异的 Agent，就是合并候选。
2. **任务包写作**：为一个真实的验收任务写一份完整任务包（objective / scope / inputs / constraints / output_contract / stop_conditions），交给同事或另一个 Agent 试跑，观察它返回 BLOCKED 还是给出结论。
3. **预埋缺陷实验**：参考第 10 节的预埋缺陷清单，在你的项目里埋入 1–2 个，观察 Reviewer Agent 能否在不看实现过程的情况下抓到。
