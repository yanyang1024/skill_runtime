...
def make(out):
    ...
    # V2 fixtures: explicit phase, origin, direct skill binding; names are fictional.
    for i,s in enumerate(sessions[:8]):
        s["org_section"]=s["dept"]
        cap="demo-analysis" if i%2 else "demo-parser"
        s["capability_events"]=[{"event_id":f"load-{i}","kind":"skill","capability_id":cap,"version":"v1",
            "action":"load","event_source":"runtime","success":True,"ts":s["start_at"]},
            {"event_id":f"invoke-{i}","kind":"skill","capability_id":cap,"version":"v1",
            "action":"invoke","event_source":"runtime","success":i%3!=0,"ts":s["end_at"]}]
        phase="development" if i<3 else "production" if i<6 else "unknown"
        s["tool_events"].append({"event_id":f"diagnostic-{i}","name":"bash" if i%2 else "custom_measurement",
            "tool_origin":"native" if i%2 else "custom","origin_source":"registry","tool_version":"v1",
            "usage_phase":phase,"phase_source":"manifest","underlying_target":"demo_parser.py",
            "capability":{"kind":"skill","id":cap,"version":"v1"},"status":"error" if i%3==0 else "success",
            "error_kind":"invalid_input" if i%3==0 else None,"ts":s["start_at"],
            "expected_error":i==0,"expectation_source":"test_definition" if i==0 else None,
            "assertion_passed":True if i==0 else None})
    sessions[0]["capability_events"].append({"event_id":"agent-run-1","kind":"agent","capability_id":"demo-checker",
        "version":"v1","action":"invoke","event_source":"runtime","success":True,"ts":sessions[0]["end_at"]})
    for i in (1,2):
        sessions[i]["artifact_events"].append({"event_id":f"common-write-{i}","op":"write","success":True,
            "sha256":"a"*64,"sha256_source":"file_bytes","size_bytes":12,"ts":sessions[i]["end_at"]})
    sessions[4]["artifact_events"].append({"event_id":"ambiguous-upload","op":"upload","success":True,
        "sha256":"a"*64,"sha256_source":"file_bytes","size_bytes":12,"ts":sessions[4]["end_at"]})
    sessions[3]["artifact_events"].append({"event_id":"lineage-upload","op":"upload","success":True,
        "artifact_id":"upload/new-id","version":"v1","source_artifact_id":"DEMO/asset-1","source_version":"sha256:v1",
        "lineage_source":"runtime","ts":sessions[3]["end_at"]})
    revisions={case_id(s):digest(s) for s in sessions}
    for r in reviews+curated:
        r["source_revision"]=revisions[r["case_id"]]
        r["label_source"]="human"
    reviews.append({"review_id":"demo-auto","case_id":case_id(sessions[0]),"source_revision":digest(sessions[0]),
        "reviewer_id":"AUTO:heuristic-v1","label_source":"heuristic","reviewed_at":"2026-08-25T10:00:00+08:00",
        "outcome":"usable","adoption":"used","note":"Demonstrates that AUTO is excluded from confirmed adoption."})
    catalog=[{"tenant_id":"DEMO","kind":"skill","capability_id":cap,"version":"v1","provider_org":"演示研发",
        "visibility_source":"registry","published_at":"2026-07-01T00:00:00+08:00","visible_to_orgs":["演示工艺","演示研发"]}
        for cap in ("demo-parser","demo-analysis","demo-unused")]
    catalog.append({"tenant_id":"DEMO","kind":"agent","capability_id":"demo-checker","version":"v1","provider_org":"演示研发"})
    write_jsonl(out/"capability_catalog.jsonl",catalog)
    write_jsonl(out/"sessions.jsonl",sessions)
    write_jsonl(out/"reviews.jsonl",reviews)
    write_jsonl(out/"curated.jsonl",curated)
    write_jsonl(out/"task_labels.jsonl",[{"case_id":r["case_id"],"source_revision":r["source_revision"],
        "source_group":r["source_group"],"task_type":r["target"]["intent"] if r["task_type"]=="intent_routing" else r["task_type"],
        "label_source":"human","reviewer_id":"demo-human"} for r in curated])
