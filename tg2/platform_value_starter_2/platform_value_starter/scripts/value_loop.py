...
def validate(row):
    ...
    for event in row.get("artifact_events", []):
        if not all(event.get(k) for k in ("event_id", "ts")):
            raise ValueError("artifact event needs event id and timestamp")
        ...
        if not (event.get("artifact_id") and event.get("version")) and not event.get("sha256"):
            raise ValueError("artifact needs stable identity/version or a byte hash")
        timestamp(event["ts"])
    for event in row.get("capability_events", []):
        if not all(event.get(k) for k in ("event_id", "kind", "capability_id", "ts")):
            raise ValueError("capability event needs kind, identity and timestamp")
        if event["kind"] not in {"skill", "agent"} or event.get("action") not in {"load", "invoke", "mention"}:
            raise ValueError("invalid capability kind/action")
        if event.get("success") is not None and type(event["success"]) is not bool:
            raise ValueError("capability success must be bool/null")
        timestamp(event["ts"])
...
def add_reviews(db, rows):
    with db:
        for r in rows:
            ...
            if label_source(r) == "human" and r.get("adoption") == "used" and not r.get("work_item_id"):
...
def snapshot(db):
    ...
    for (body,) in db.execute("SELECT body FROM reviews"):
        ...
        channel = latest if label_source(r) == "human" else automatic
        if key not in channel or rank > channel[key][0]:
            channel[key] = (rank, r)
    ...
    for cid, rev, body in db.execute("SELECT case_id,revision,body FROM sessions ORDER BY case_id"):
        ...
        rows.append({"case_id": cid, "source_revision": rev, "session": s,
                     ...
                     "auto_assessment": automatic.get((cid,rev),(None,None))[1],
...
def token_rows(cases, start, end):
    ...
    """
    observations, seen, request_observations = [], {}, {}
    excluded = Counter()
    for c in cases:
        s = c["session"]
        if s.get("requests"):
            ...
            for r in s["requests"]:
                ...
                if key in seen:
                    ...
                    previous = request_observations.get(key)
                    if previous:
                        previous["source_case_ids"].append(c["case_id"])
                        if previous["dept"] != (s.get("org_section") or s.get("dept") or "unknown"):
                            previous["dept"] = "unknown"
                            previous["attribution_note"] = "conflicting_projection_org"
                    continue
                seen[key] = digest(r)
                if start <= timestamp(r["ts"]) < end:
                    observation = {**r,"tenant_id":s["tenant_id"], "case_id": c["case_id"],"source_case_ids":[c["case_id"]],
                        "dept": s.get("org_section") or s.get("dept") or "unknown", "scope": "request"}
                    request_observations[key] = observation
                    observations.append(observation)
                else:
                    excluded["unique_requests_outside_window"] += 1
        else:
            ...
            if a and b and start <= a <= b < end and stats.get("usage_scope") == "session_exclusive":
                if stats.get("input_tokens") is not None or stats.get("output_tokens") is not None:
                    observations.append({**stats,"tenant_id":s["tenant_id"], "case_id": c["case_id"], "dept": s.get("org_section") or s.get("dept") or "unknown", "scope": "session_summary"})
                    continue
    ...
    for c in cases:
        ...
        for e in s.get("artifact_events", []):
            ...
            if eid in seen:
                ...
                continue
            seen[eid] = digest(e)
            if not e["success"] or e["op"] not in {"write","read"} or not (e.get("artifact_id") and e.get("version")):
                continue
...
def report(cases, start, end, out, costs=None):
    ...
    for c in cases:
        ...
        if s.get("user_id") and any(m["role"] == "user" and timestamp(m.get("ts")) and start <= timestamp(m["ts"]) < end for m in s.get("messages", [])):
            ...
            active.add(key)
            dept_users[s.get("org_section") or s.get("dept") or "unknown"].add(key)
    ...
    from resource_diagnostics import asset_relations
    all_relations,_ = asset_relations(cases,start,end)
    edges = [e for e in all_relations if e["operation"]=="read" and e["source_attribution_confirmed"]]
    ...
    stats = {"metric_version": METRIC_VERSION, "window": [start.isoformat(), end.isoformat()],
             ...
             "auto_assessed_sessions": sum(bool(c.get("auto_assessment")) for c in cohort),
    ...
    lines = ["# 平台价值证据报告", "", f"窗口：[{start.isoformat()}, {end.isoformat()})；口径 {METRIC_VERSION}。",
             ...
             f"| 自动候选覆盖 | {rate(stats['auto_assessed_sessions'], len(cohort))} | AUTO/模型/规则不计入人工结果确认 |",
    ...
    write_jsonl(Path(out) / "usage_ledger.jsonl", tokens)
