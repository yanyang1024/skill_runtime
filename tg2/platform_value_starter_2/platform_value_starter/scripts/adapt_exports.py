...
def convert(raw, meta):
    ss = raw.get("session", {})
    stats = raw.get("stats", {})
    if not isinstance(ss,dict) or not isinstance(stats,dict):
        raise ValueError("session/stats must be objects")
    if not any(k in raw for k in ("messages","stats","requests","tool_events","capability_events")):
        raise ValueError("not a session detail record; a list/index item needs a detail adapter")
    warnings = []
    if isinstance(raw.get("summary"),dict):
        warnings.append("structured_summary_preserved_as_metadata_not_text")
    messages = []
    if not isinstance(raw.get("messages",[]),list) or not all(isinstance(m,dict) for m in raw.get("messages",[])):
        raise ValueError("messages must be a list of objects; add an explicit adapter")
    ...
    result = {
        ...
        "org_section": raw.get("org_section") or meta.get("org_section") or raw.get("dept") or meta.get("dept"),
        ...
        "capability_events": raw.get("capability_events", []),
        "summary_metadata": raw.get("summary") if isinstance(raw.get("summary"),dict) else None,
...
def main():
    ...
    for meta in read_jsonl(a.manifest):
        try:
            ...
            if not raw_rows:
                raise ValueError("empty export: zero sessions, not a successfully parsed session file")
            converted = []
            local_ids = set()
            for raw in raw_rows:
