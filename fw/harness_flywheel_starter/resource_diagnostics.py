...
def tool_summary(cases, start, end, min_calls=20):
    ...
    for x in observed:
        ...
        count["assertions_observed"] += int(e.get("expectation_source") == "test_definition" and isinstance(e.get("assertion_passed"), bool))
    ...
    for key,g in sorted(groups.items()):
        ...
        rows.append(dict(zip(("tenant_id","org","phase","origin","tool_id","tool_version","capability_kind","capability_id","capability_version"),key),
            **dict(n), known_calls=known, non_expected_test_calls=eligible, unexpected_errors=unexpected,
            ...
            error_interpretation="unexpected_errors 是未被确认为预期负例的 error 计数；缺测试元数据时不证明错误非预期或任务失败",
...
def capability_summary(cases, catalog, start, end):
    ...
    for key,cat in sorted(catalog_by_key.items()):
        ...
        if cat.get("publication_source") not in TRUSTED_METADATA or cat.get("usage_event_semantics") != "invoke_required":
            continue  # First seen is not published; a load-only skill has no invocation denominator.
