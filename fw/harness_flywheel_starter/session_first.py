#!/usr/bin/env python3
"""会话优先的启发性入口：没有 branch/version 也能产生候选，不升级为根因或验收。"""
import argparse
import posixpath
import re
...
from resource_diagnostics import TRUSTED_METADATA, origin_of, phase_of

VERSION = "session-first-v1.1"


def tool_part_to_event(part, *, event_id, ts, **metadata):
    """接到你自己的 adapter 中；ts 用带时区的完成时间，event_id 沿用原始 ID。

    只示意反馈中 state.input/output/error 的映射，不假装支持全部原始导出格式。
