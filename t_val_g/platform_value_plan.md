...
|---|---|---|---|---|
| P0 | 统一字段与证据口径 | 先阻止“算得出来但名字错了”的汇报 | 1–2 天适配导出与抽查 | 抽 10 条覆盖 MD、JSON、工具失败、无文件问答；能回溯且未知不消失即可 |
| P0 | 每周复核 10–20 条，形成采用卡与失败清单 | 同时服务领导与用户，一次确认可复用 | 每周约 30–60 分钟，先试再调整 | 3–5 个可被业务方核对的案例，1 个明确改进动作 |
| P0 | 修复 1 个高频工具/skill 问题，补回归题 | 这是最容易产生可证明增量的地方 | 根据错误来源，可能只需半天至数天 | 相同任务、数据与模型下，失败转通过且没有明显退步 |
| P1 | 冻结 20–30 个自包含任务 | 引入新模型时有自己的准绳 | 先选 2–3 个高频、可判定任务族 | 当前模型基线、候选差异、退步题清单；不追求一次覆盖整个企业 |
| P1 | CPU 任务分类基线 | 验证是否需要让大模型做每个简单判别 | 数百条独立来源人工标签更适合起步；少量先试 | 与规则/多数类比较；不足就保留规则，不强行升级模型 |
| P2 | 狭窄抽取任务的合成 + 小规模 SFT/LoRA | 当规则/提示已到瓶颈且调用频繁时再尝试 | 冻结真实保留集、训练许可、离线模型与算力 | 在真实保留集上有增量，成本摊销合理才继续 |

**暂缓**：通用会话全量训练、自动偏好对、跨部门价值排名、复杂价值象限、职级推断、全量“半导体专家模型”、全功能数据平台。先用一个 SQLite 文件和版本化文件目录足够。
