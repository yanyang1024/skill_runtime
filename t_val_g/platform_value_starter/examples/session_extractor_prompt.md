...
用途：给复核员预填任务与失败线索。输出是候选，不直接修改 outcome、business_value 或训练许可。
...
先用十几条人工案例校准，重点看错把需求变更当模型失败、漏掉无文件问答、捏造完成证据等问题。模型输出的证据 ID 必须在输入中存在；不通过结构/引用检查就退回人工。可以只对抽样案例调用内网模型，初版不必对全部历史跑昂贵分析。
