...
import type { ChatSSEEvent } from "../app/web/src/types/chat.js";
...
describe("opencode client: x-opencode-directory header", () => {
  ...
  function lastHeader(name: string): string | string[] | undefined {
...
describe("chatApi: SSE event parsing", () => {
  it("parses text_delta", () => {
    const event = parseChatSSEEvent(
      JSON.stringify({ type: "text_delta", message_id: "m1", part_id: "p1", content: "hi" }),
    );
...
describe("sse normalizer: OpenCode raw → ChatSSEEvent", () => {
  function emptyState(): {
    ...
    return {
      ...
      partTypes: new Map<string, "text" | "reasoning" | "tool" | "error">(),
  ...
  it("tool.start / tool.delta / tool.end → tool events", () => {
    const state = emptyState();
    state.currentMessageId = "m1";
    const start = normalizeOpenCodeEvent(
      "tool.start",
      { id: "t1", toolName: "edit_file", input: { path: "x.md" } },
      state,
    );
    assert.ok(start);
  ...
  it("question.asked → question", () => {
    ...
    const events = normalizeOpenCodeEvent(
      "question.asked",
      { id: "q-1", content: "选择模型", kind: "choice", options: [{ id: "a", label: "A" }] },
      state,
    );
    assert.ok(events);
    const arr = Array.isArray(events) ? events : [events];
