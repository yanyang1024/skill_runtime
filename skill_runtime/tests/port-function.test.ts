...
import {
  ...
  stageOutputDir,
  runDir,
} from "../app/shared/utils/paths.js";
...
async function postJson(url: string, body: unknown, auth?: { username: string; password: string }) {
...
async function getJson(url: string, auth?: { username: string; password: string }) {
...
function startAppServer(): Promise<{ port: number; proc: ReturnType<typeof spawn> }> {
  return new Promise((resolve, reject) => {
...
async function cleanupSkillRun(skillId: string, runId?: string) {
...
describe("opencode server", () => {
  ...
  it("can create a session", async (t) => {
    ...
    const resp = await postJson(
      `${OPENCODE_BASE_URL}/session`,
      { title: "port-function-test" },
...
describe("skill-growth app api", () => {
  let server: { port: number; proc: ReturnType<typeof spawn> } | null = null;
  ...
  it("can commit work/ to preview", async () => {
    ...
    await fs.writeFile(
      path.join(stageWorkDir(previewRunId, "grow-build", 1), "SKILL.md"),
      "# Preview Skill\n\nupdated\n",
      "utf-8",
    );

    const commitResp = await postJson(
      api(`/runs/${previewRunId}/stage/grow-build/commit`),
      { attempt: 1 },
    );
  ...
  it("can run api-tests and produce machine-test-result.json", async () => {
    ...
    await fs.writeFile(
      path.join(testDir, "test_sample.sh"),
      "#!/bin/bash\necho '{\"status\":\"ok\"}'\nexit 0\n",
      { mode: 0o755 },
    );

    const runnerResp = await postJson(
      api(`/runs/${apiRunId}/stage/observe-api-scan/run-api-tests`),
      { attempt: 1 },
    );
    assert.equal(runnerResp.status, 200);
    const runnerData = (await runnerResp.json()) as any;
