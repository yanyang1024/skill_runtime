...
import { assertSafeIdentifier, sanitizePathComponent, resolveContainedPath, safeResolve, PathSecurityError, } from "../app/shared/utils/security.js";
describe("security: identifier validation", () => {
    ...
    it("rejects empty identifier", () => {
        ...
        assert.throws(() => assertSafeIdentifier(undefined, "run"), PathSecurityError);
    });
...
describe("security: safeResolve with symlinks", () => {
    ...
    it("rejects symlink escape", async () => {
        ...
        await assert.rejects(() => safeResolve(path.join(root, "workspace"), "sub/escape.txt"), PathSecurityError);
        await fs.rm(outside);
    });
...
describe("security: artifact route blocks symlink escape", () => {
    it("returns 403 when artifact path is a symlink outside output dir", async () => {
        ...
        try {
            const res = await fetch(`http://127.0.0.1:${port}/api/runs/${runId}/stage/${stageId}/artifact/escape.txt?attempt=${attempt}`);
