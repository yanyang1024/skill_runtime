...
export function MessageBubble({ message }) {
    ...
    return (<div className={`message-bubble ${isUser ? "user" : "assistant"}`}>
      ...
      <div className="message-content">
        {message.parts.map((part) => (<PartRenderer key={part.id} part={part}/>))}
      </div>
    </div>);
}
function PartRenderer({ part }) {
    switch (part.type) {
        ...
        case "tool":
            return (<div className="tool-part">
          <div className="tool-name">🔧 {part.toolName || "tool"}</div>
          {part.input && (<pre className="tool-input">{JSON.stringify(part.input, null, 2)}</pre>)}
          {part.content && <pre className="tool-output">{part.content}</pre>}
          {part.output && (<pre className="tool-output">{JSON.stringify(part.output, null, 2)}</pre>)}
          {part.status === "error" && <div className="tool-error">执行失败</div>}
        </div>);
        case "file":
            return (<div className="file-part">
