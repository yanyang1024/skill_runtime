...
function scopeKey(runId, stageId, attempt) {
    return `${runId}|${stageId}|${attempt}`;
}
async function scanMtimes(dir) {
    const map = new Map();
    try {
 ...
 * 返回一个停止监听的函数。
 */
export async function watchStageOutput(runId, stageId, attempt, onChange) {
...
export function unwatchStageOutput(runId, stageId, attempt) {
    const key = scopeKey(runId, stageId, attempt);
    const entry = watchers.get(key);
    if (entry) {
        entry.watcher.close();
...
export async function stopAllArtifactWatchers() {
...
//# sourceMappingURL=artifactWatcher.js.map