...
export async function runGrowDryRun(skillId, trace) {
...
function buildProposalMarkdown(skillId, trace, plan) {
    const ops = plan.planned_operations
        .map((op) => `- **${op.op_id}** (${op.type}): \`${op.target}\` — ${op.reason}（风险: ${op.risk}）`)
        .join("\n");
    return [
        "# Growth Proposal",
        "",
