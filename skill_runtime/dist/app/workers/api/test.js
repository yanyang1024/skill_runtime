...
export async function runApiTest(skillId, endpointId, baseURL = process.env.SKILL_GROWTH_API_BASE_URL ?? "http://localhost:3000/mock") {
    ...
    if (tests) {
        ...
        if (existence)
            existence.status = results[0].passed ? "passed" : "failed";
        if (schema)
            schema.status = results[1].passed ? "passed" : "failed";
    }
