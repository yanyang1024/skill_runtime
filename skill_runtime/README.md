...
- **SSE 归一化**：Backend 将 OpenCode 原始 SSE 事件转换为统一的 `ChatSSEEvent` 协议，前端指数退避重连
- **Token 预算监控**：实时显示上下文窗口使用率，60%/80%/100% 三级颜色预警
- **上下文可见性**：Start 启动时展示模型可访问的全部文件清单
- **阶段流转文件传递**：`carry_outputs` 精确按文件复制，director-review 双路径传递
- **文件上传/下载**：ChatInput 附件上传 + ArtifactPanel 一键下载
...
| `SKILL_GROWTH_DEFAULT_MODEL` | OpenCode 默认模型 | `local-v1/qwen3.5:9b` |
| `SKILL_GROWTH_SMALL_MODEL` | OpenCode 轻量模型 | `local-v1/qwen3.5:9b` |
