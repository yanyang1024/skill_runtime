...
| OpenCode CLI | 可选；实际启动 stage runtime 时需要（`opencode web` / `opencode serve`） |
...
2. 点击 **启动 Stage**：应用会构建 workspace、生成 `opencode.json`、分配端口并启动 `opencode web`
3. 点击 **打开 OpenCode Web**：在新标签页进入该阶段的 OpenCode 工作现场
4. 右侧 **Prompt Assistant** 会自动生成推荐输入；可编辑后点击“发送到 OpenCode”
5. 底部 **Artifacts** 会自动轮询并展示 `output/` 下生成的 Markdown 产物
...
```bash
# promote preview -> stable（会先做 stable 快照）
...
Prompt Recommender 默认调用 `http://172.24.16.1:11434/v1/chat/completions`，模型 `glm4:9b`。如需修改，编辑：

- `app/prompt_recommender/client.ts` 中的 `baseURL` / `apiKey`
- `recommendPrompt` 调用处传入的 `model` 参数
...
| POST | `/api/runs/:run/stage/:stage/message` | 发送消息到 OpenCode runtime |
...
| GET/POST | `/api/runs/:run/stage/:stage/view/*` | 反向代理 OpenCode Web UI |
