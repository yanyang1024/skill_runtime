...
export const securityDecode = self => {
  switch (self._tag) {
    case "Http":
      {
        return Effect.map(HttpServerRequest,
        // schemeLength + space
        request => Redacted.make((request.headers.authorization ?? "").slice(self.schemeLength + 1)));
      }
    case "ApiKey":
      {
    ...
    case "Basic":
      {
        ...
        return HttpServerRequest.pipe(Effect.flatMap(request => Effect.fromResult(Encoding.decodeBase64String((request.headers.authorization ?? "").slice(basicLen)))), Effect.match({
          onFailure: () => empty,
          onSuccess: header => {
