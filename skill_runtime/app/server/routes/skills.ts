...
import { skillRoot, skillStableDir, growthRunsDir, backupsDir, toPosix } from "../../shared/utils/paths.js";
...
async function buildTree(absPath: string, skillRootPath: string): Promise<TreeNode[]> {
...
async function readLatestTrace(skillId: string) {
  const runsDir = growthRunsDir(skillId);
  const runs = await fs.readdir(runsDir);
  const traceFiles: { run: string; mtime: number }[] = [];
...
async function readLatestDryRunPlan(skillId: string) {
  const runsDir = growthRunsDir(skillId);
  const runs = await fs.readdir(runsDir);
  const planFiles: { run: string; mtime: number }[] = [];
  for (const run of runs) {
