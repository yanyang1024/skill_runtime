...
async function applyCgroupLimits(serverId: string, pid: number): Promise<void> {
  if (process.env.STAGE_CGROUP !== "1") return;
...
function startHealthMonitor(runtime: RunningStage, interval = 30000) {
  ...
  (runtime as unknown as Record<string, unknown>).health_timer = timer;
}
...
export async function startStageRuntime(opts: StartStageRuntimeOptions): Promise<OpencodeRuntime> {
  ...
  if (proc.pid) void applyCgroupLimits(serverId, proc.pid);
