...
export default function App() {
  ...
  const [currentStageId, setCurrentStageId] = useState<string | null>("observe-log-review");
  ...
  useEffect(() => {
    listSkills().then((s) => {
      setSkills(s);
      if (s.length > 0 && !currentSkill) setCurrentSkill(s[0]);
    });
    listRuns().then((r) => {
      setRuns(r);
      if (r.length > 0 && !currentRunId) setCurrentRunId(r[r.length - 1].run_id);
    });
