...
import type { EndpointManifest as EndpointManifestType } from "../../shared/schemas/index.js";
...
export async function runApiScan(skillId: string): Promise<EndpointManifestType> {
  ...
  let manifest: EndpointManifestType = {
    skill_id: skillId,
...
function parseEndpoints(content: string, sourceFile: string): DiscoveredEndpoint[] {
  ...
  while ((match = methodPathRe.exec(content)) !== null) {
    const method = match[1] as DiscoveredEndpoint["method"];
    const p = match[2];
    if (!p) continue;
    const id = p
      .replace(/\//g, "_")
