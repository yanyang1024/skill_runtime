...
import type { QualityReport, QualityCheckResult } from "../../shared/schemas/index.js";
...
export async function runQualityGate(
  ...
  try {
    ...
    for (const ref of refs) {
      ...
      if (content.trim().length === 0) referencesOk = false;
    }
  } catch {
  ...
  const hasPositiveGuidance =
    skillMd.includes("先完成全量分析") && skillMd.includes("不在分析中间停顿提问");
  results.push({
