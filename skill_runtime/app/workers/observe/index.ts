...
import type { RuntimeTrace, RuntimeReplayCard, GrowthOpportunities } from "../../shared/schemas/index.js";
...
export async function runObserve(skillId: string): Promise<{ runId: string; trace: RuntimeTrace }> {
  const evalPath = path.join(skillStableDir(skillId), "evals", "evals.json");
  const raw = await fs.readFile(evalPath, "utf-8");
  const evals: { skill_name: string; description: string; evals: EvalItem[] } = JSON.parse(raw);
  ...
  const replayCard: RuntimeReplayCard = {
    card_id: `rc-${Date.now()}`,
  ...
  const opportunities: GrowthOpportunities = {
    ...
    opportunities: trace.growth_candidates.map((c, i) => ({
      id: `opp-${i + 1}`,
      type: c.type as GrowthOpportunities["opportunities"][number]["type"],
      summary: c.summary,
...
function buildReplayCardMarkdown(trace: RuntimeTrace, evals: EvalItem[]): string {
  const lines = [
    "# Runtime Replay Card",
    "",
    "## 1. 本次会话学到了什么",
