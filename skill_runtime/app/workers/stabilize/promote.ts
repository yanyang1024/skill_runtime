...
export async function runStabilizePromote(
  skillId: string,
  previewId?: string,
  ...
  if (previews.length === 0) throw new Error("no preview to promote");
  const targetPreviewId = previewId ?? previews.sort().reverse()[0];
  if (!targetPreviewId) throw new Error("could not determine preview id");
  ...
  let previewManifest: { quality_report_id?: string } = {};
  try {
  ...
  await fs.writeFile(
    skillYamlPath,
    YAML.stringify({ name: skillId, version: version.replace(/^v/, ""), promoted_at: utcTimestamp() }),
    "utf-8",
  );
  ...
  await fs.writeFile(
    path.join(releaseDir, "snapshot-manifest.yaml"),
...
function buildChangelog(
  ...
  previewId: string,
  snapshotId: string,
  previewManifest: { quality_report_id?: string },
): string {
