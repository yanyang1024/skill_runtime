...
export async function findSnapshotManifest(
  skillId: string,
  snapshotId: string,
  ...
  try {
    ...
    for (const f of files) {
      if (f.endsWith(".manifest.yaml")) candidates.push(path.join(stableDir, f));
    }
  } catch {
    // ignore
  }
  ...
  try {
    ...
    for (const pid of previewIds) {
      ...
      for (const f of files) {
        if (f.endsWith(".manifest.yaml")) candidates.push(path.join(dir, f));
      }
    }
  } catch {
    // ignore
  ...
  for (const p of candidates) {
    try {
      ...
      const manifest = YAML.parse(raw) as SnapshotManifest;
      if (manifest.snapshot_id === snapshotId) return manifest;
    } catch {
      // ignore
    }
  }
...
export async function rollbackSkill(skillId: string, snapshotId: string): Promise<void> {
