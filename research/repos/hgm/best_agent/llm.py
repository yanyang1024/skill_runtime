...
def create_client(model: str):
    if 'gpt' in model or model.startswith("o"):
        ...
        return openai.OpenAI(base_url=f"http://{model[11:]}:8000/v1", api_key="dummy"), model
    else:
       print(f"Using OpenRouter API with model {model}.")
       return openai.OpenAI(base_url="https://openrouter.ai/api/v1", api_key=os.getenv('OpenRouter_API_KEY')), model
...
def get_json_response_from_llm(msg,
        client,
        model,
        system_message,
        ):
