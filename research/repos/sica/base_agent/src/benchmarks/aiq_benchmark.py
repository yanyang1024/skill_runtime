...
"""
AIQ Benchmark Implementation

This benchmark evaluates an agent's ability to improve software projects
following the AI Quality (AIQ) protocol v5.0, which defines quantifiable measures
...
@dataclass
class AIQBenchmarkProblem(Problem):
    """Problem subclass for the AIQ benchmark
...
class AIQBenchmark(BaseBenchmark):
    """
    Benchmark for evaluating agent capability in improving software through the AIQ protocol.
    ...
    def __init__(self, seed: int = 2, subset_size: int | None = None):
        """
        Initialize the AIQ benchmark

        Args:
            seed: Random seed for consistent project selection
            subset_size: Number of projects to use (default: None = all)
        """
        ...
        for project in PROJECT_LIST:
            self._all_problems.append(
                AIQBenchmarkProblem(
                    problem_id=project,
                    statement="Following the AIQ v5.0 protocol, improve this project as much as you can within the budget. "
                             "Run the quality.py script to measure your progress and use its CLI options for analysis.",
        ...
        if subset_size is not None:
            random.seed(seed)
            self._all_problems = random.sample(self._all_problems, min(subset_size, len(self._all_problems)))

    @property
    def problems(self) -> list[Problem]:
        """Return list of AIQ problems"""
    ...
    async def setup_problem(
        ...
        """
        if not isinstance(problem, AIQBenchmarkProblem):
            raise ValueError("Problem must be an AIQBenchmarkProblem instance")

        aiq_problem: AIQBenchmarkProblem = problem
    ...
    async def score_problem(
        ...
        """
        if not isinstance(problem, AIQBenchmarkProblem):
            logger.error("Not an AIQ problem")
            return 0.0, "Not an AIQ problem", None
