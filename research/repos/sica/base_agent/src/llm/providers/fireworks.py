...
"""Fireworks-specific LLM provider implementation."""
...
# from openai.resources.chat.completions import Headers
...
from ...utils.parsing import extract_after_last, extract_between_patterns
...
class FireworksProvider(BaseProvider):
    """Provider implementation for the Fireworks AI provider."""
    ...
    def map_stop_reason(self, response: Any) -> tuple[StopReason, Optional[str]]:
        """Map Fireworks finish reasons to our standard format."""
        #
        # TODO: update this based on observed responses
        #
    ...
    def _create_token_usage(
        self, response: Any, response_headers
    ) -> TokenUsage:
        """Create TokenUsage object from response."""
        usage = getattr(response, "usage", None)
        if not usage:
            logger.warning("Token usage not reported, defaulting to 0")
            return TokenUsage()

        cached_prompt_tokens = response_headers.get("fireworks-cached-prompt-tokens", 0)

        try:
            return TokenUsage(
                uncached_prompt_tokens=getattr(usage, "prompt_tokens", 0),
                cache_write_prompt_tokens=0,  # TODO: Try to infer this...
    ...
    def _prepare_messages(self, messages: list[Message]) -> list[dict | ChatCompletionMessage]:
        ...
        for msg in messages:
            if msg.role == "assistant":
                ...
                fw_messages.append(ChatCompletionMessage(
                    content=msg_content,
                    role="assistant",
                ...
                for block in msg.content:
                    if isinstance(block, TextContent):
                        ...
                        if msg_content != "":
                            fw_messages.append({"role": msg.role, "content": msg_content})
                            msg_content = ""
                        fw_messages.append({"role": "tool", "tool_call_id": block.call_id, "content": block.content})
                if msg_content != "":
                    fw_messages.append({"role": msg.role, "content": msg_content})
    ...
    async def create_completion(
        ...
        # NOTE: I think this is deepseek specific
        ...
        token_usage = self._create_token_usage(response, raw_response.headers)
        ...
        response_content = []
        if model == Model.FIREWORKS_DEEPSEEK_R1:
            reasoning_content = extract_between_patterns(
                message.content or "", "<think>", "</think>"
            )
            reasoning_content = reasoning_content.strip() if reasoning_content else ""
            response_content.append(ReasoningContent(text=reasoning_content))

            message_content = extract_after_last(message.content or "", "</think>").lstrip()
            response_content.append(TextContent(text=message_content))
        else:
            response_content.append(TextContent(text=message.content or ""))
    ...
    async def create_streaming_completion(
        ...
        #
        # TODO: update this to use the new framework constructs (refer to
        # create_completion)
        #
        start_time = datetime.now()
